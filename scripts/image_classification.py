    ...
    for epoch in range(max_epochs):
        for inputs, targets in train_loader:
            ...
            if train_step % save_scores_every_n_steps == 0:
                ...
                id_scores[train_step].to_pickle(
                    scores_dir / f"id_scores_step={train_step}.pkl"
                )
                ood_scores[train_step] = _compute_test_scores(net, device, ood_test_loader)
                ood_scores[train_step].to_pickle(
                    scores_dir / f"ood_scores_step={train_step}.pkl"
                )
...
def train_and_inference(
    ...
    config_path = logs_dir / f"config_me={max_epochs}_bs={batch_size}_lr={learning_rate}.yaml"
    ...
    if (scores_dir / "results_id_last_step.csv").exists() and (scores_dir / "results_ood_last_step.csv").exists():
        ...
        id_scores = {
            int(scores_path.stem.split("step=")[-1]): pd.read_pickle(scores_path)
            for scores_path in scores_dir.glob("id_scores_step=*.pkl")
        }
        id_labels_df = pd.read_csv(scores_dir / "id_test_labels.csv", index_col=None, header=None)
        
        ood_scores = {
            int(scores_path.stem.split("step=")[-1]): pd.read_pickle(scores_path)
            for scores_path in scores_dir.glob("ood_scores_step=*.pkl")
        }
