...
def load_dataset(dataset, batch_size):
    ...
    val_set = DatasetClass(root="./data", train=False, download=True, transform=transform)
...
def _train_and_inference(
    ...
    val_labels_df.to_csv(scores_dir / f"labels.csv", index=False, header=False)
    ...
    for epoch in range(max_epochs):
        for inputs, targets in train_loader:
            ...
            if train_step % save_scores_every_n_steps == 0:
                ...
                all_scores[train_step].to_csv(scores_dir / f"scores_step={train_step}.csv", index=False, header=False)
            ...
            if train_step % log_train_loss_every_n_steps == 0:
                print(f"Epoch: {epoch}, Train step: {train_step}, Loss: {train_loss.item():.4f}")
    ...
    results = results.T.reset_index().rename(columns={"index": "metric", 0: "value"}).set_index("metric")
    results.to_csv(scores_dir / f"results_last_step.csv", index=False)
...
def train_and_inference(
    ...
    if (scores_dir / f"results_last_step.csv").exists():
        print(f"Scores already exist at {scores_dir}. Skipping training.")
        all_scores = {
            int(scores_path.stem.split("step=")[-1]): pd.read_csv(scores_path, index_col=None, header=None)
            for scores_path in scores_dir.glob("scores_step=*.csv")
        }
        labels_df = pd.read_csv(scores_dir / f"labels.csv", index_col=None, header=None)
...
def compute_metrics(scores, labels, metrics):
    ...
    results = pd.DataFrame(results).pivot(index="step", columns="metric", values="value")
...
def plot_results(results, loss, dataset, model, output_dir: Path):
    ...
    ax.set_title(f"Validation curves when trained with {loss_display}\nDataset: {dataset_display}, Model: {model_display}")
    ax.set_xlabel("Training Step")
    ax.set_ylabel("Metric Value")
    ax.grid()
    ax.legend()
    fig.savefig(output_dir / f"training.pdf", bbox_inches="tight", dpi=300)
    plt.close(fig)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save-scores-every-n-steps", type=int, default=10, help="Save scores every n training steps")
    parser.add_argument("--log-train-loss-every-n-steps", type=int, default=10, help="Log training loss every n training steps")
    parser.add_argument("--loss", type=str, default="cls_cross_entropy", help="Loss function to use")
    parser.add_argument("--eval-metrics", nargs="+", default=EVAL_METRICS, help="Evaluation metrics to compute")
    parser.add_argument("--logs-dir", type=str, default="scores/image_classification", help="Directory to save logs and scores")
    parser.add_argument("--output-dir", type=str, default="outputs/image_classification", help="Directory to save output plots")
