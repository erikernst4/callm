...
def compute_confidence(cls, confidences: torch.Tensor, correctness: torch.Tensor, **kwargs) -> float:
    metric = cls(**kwargs)
    metric.update(confidences, correctness)
    return metric.compute().item()

CONFIDENCE_METRICS = {
    "ece": lambda confidences, correctness: compute_confidence(ExpectedCalibrationError, confidences, correctness, n_bins=10),
    "brier": lambda confidences, correctness: compute_confidence(BrierScore, confidences, correctness),
    "cross-entropy": lambda confidences, correctness: compute_confidence(CrossEntropy, confidences, correctness),
    "auc": lambda confidences, correctness: compute_confidence(AUCScore, confidences, correctness),
...
def generate_confidences(num_samples, priors, N=5, sigma_expand=1.0, suboptimal_T=1.0):
    ...
    for k_sampled in torch.randint(0, K, (num_samples,)):
        ...
        for kk in range(K):
            for nn in range(N):
                p_x_given_n_k[nn, kk] = torch.exp(-0.5 * (torch.sum((x - mu[kk][nn])**2) / sigma[kk]**2)) / (sigma[kk] * torch.sqrt(torch.tensor(2.) * torch.pi))
        p_n_k_given_x = p_x_given_n_k * p_n_given_k * p_k / (p_x_given_n_k * p_n_given_k * p_k).sum()
    ...
    return torch.tensor(confidences), torch.tensor(correctness), torch.stack(x_sampled), mu, sigma
...
def plot_distribution(mu, sigma, x, confidences, correctness, output_dir):
    ...
    for k, (m, s) in enumerate(zip(mu, sigma)):
        for i in range(m.shape[0]):
            ax[0].scatter(m[i, 0], m[i, 1], color=f'C{k}', label=f'Gaussian {k}' if i == 0 else None, alpha=1, marker='x')
            circle = plt.Circle((m[i, 0], m[i, 1]), s, color=f'C{k}', fill=False, alpha=0.5)
            ax[0].add_artist(circle)
    ax[0].scatter(x[:, 0], x[:, 1], color='black', label='Samples', alpha=0.5, marker='x', s=1)
    ...
    ax[1].text(0.05, 0.50, f"Accuracy: {acc_result:.4f}", transform=ax[1].transAxes, fontsize=12, verticalalignment='top')

    # Compute confidence metrics
    for name, metric in CONFIDENCE_METRICS.items():
        result = metric(confidences, correctness)
        ax[1].text(0.05, 0.95 - 0.05 * list(CONFIDENCE_METRICS.keys()).index(name), f"{name}: {result:.4f}", transform=ax[1].transAxes, fontsize=12, verticalalignment='top')
...
def main(num_samples, priors, N, sigma_expand, output_dir):
    ...
    confidences, correctness, x, mu, sigma = generate_confidences(num_samples=num_samples, priors=priors, N=N, sigma_expand=sigma_expand)
