...
def relative_difference(d):
    ...
    result = (answer - eq_group) / answer if answer != 0 else 0.
    return result
...
def plot_heatmaps(K, N, metrics, output_dir, unidim, seed):
    ...
    fig, ax = plt.subplots(1, len(metrics) + 1, figsize=(5 * len(metrics), 5), sharey=True, sharex=True)
    vmin = min(rel_diff[0].min(), error_rate_results.min().min())
    vmax = max(rel_diff[0].max(), error_rate_results.max().max())
    cmap = cm.Reds
    norm = cm.colors.Normalize(vmin=vmin, vmax=vmax)
    ax[0].imshow(error_rate_results, aspect="auto", cmap=cmap, norm=norm, extent=[
        min_sigma_N, max_sigma_N, min_sigma_K, max_sigma_K
    ])
    ...
    for j, (metric, group) in enumerate(rel_diff.groupby("metric"), start=1):
        ...
        ax[j].imshow(this_heatmap, aspect="auto", cmap=cmap, norm=norm, extent=[
            min_sigma_N, max_sigma_N, min_sigma_K, max_sigma_K
        ])
