...
def main(out_dir,logs_dir):
    ...
    files = list(logs_dir.rglob("evaluation_results.csv")) + list(logs_dir.rglob("merged_results.csv"))
    ...
    dedup_df = combined_df.groupby(grp_cols, as_index=False).agg({
        'gold answers': 'first',
        'system': lambda x: sorted(list(set(x)))
    })
    ...
    filtered_df['question'] = pd.Categorical(filtered_df['question'], categories=categories, ordered=True)
    filtered_df = filtered_df.sort_values('question')
    
    # Reorder columns
    filtered_df = filtered_df[['question', 'gold answers', 'predicted answer', 'correct', 'system']]
    
    output_file = out_dir / "aggregated_results.csv"
    filtered_df.to_csv(output_file, index=False)
    print(f"Saved aggregated results to {output_file} w/ {len(filtered_df)} rows (from {len(combined_df)} raw).")
