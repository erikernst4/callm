...
def cli_main():
    ...
    if hasattr(cli.model, 'tokenizer') and hasattr(cli.model, 'model_name'):
        ...
        if cli.datamodule.model_name != cli.model.model_name:
            print(f"Warning: Data module model_name ({cli.datamodule.model_name}) doesn't match "
    ...
    # Run validation
    if cli.config.get('subcommand') == 'validate' or not hasattr(cli.config, 'subcommand'):
