...
from callm.metrics import expected_calibration_error, brier_score, cross_entropy, auc_score
...
class LLM(LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for i, (output_text, question, gold_answer_list) in enumerate(zip(outputs, questions, gold_answers)):
            # Extract answer and confidence
            pred_answer, confidence = self.extractor.extract(output_text)
            
            # Evaluate correctness
            is_correct = self.evaluator.evaluate(question, pred_answer, gold_answer_list)
    ...
    def on_validation_epoch_end(self):
        ...
        import numpy as np
        all_confidences = np.array([out["confidence"] for out in self.validation_outputs])
        ...
        try:
            with open(all_outputs_file, "w", newline="", encoding='utf-8') as f:
                writer = csv.writer(f)
                writer.writerow(["Question", "Gold Answer", "Predicted Answer", "Confidence", "Correct", "Raw Output"])
                
                for out in self.validation_outputs:
                    confidence_str = f"{out['confidence']:.6f}" if not np.isnan(out['confidence']) else "nan"
                    writer.writerow([
                        out["question"],
        ...
        if n_invalid > 0:
            ...
            try:
                with open(failure_file, "w", newline="", encoding='utf-8') as f:
                    writer = csv.writer(f)
                    writer.writerow(["Question", "Gold Answer", "Predicted Answer", "Confidence", "Raw Output", "Reason"])
                    ...
                    for idx in invalid_indices:
                        ...
                        writer.writerow([
                            out["question"],
                            out["gold_answers"][0] if out["gold_answers"] else "N/A",
                            short_output(out["pred_answer"]),  # Truncate predicted answer too
                            out["confidence"],
                            short_output(out["raw_output"]),
