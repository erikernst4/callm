...
class TriviaQADataModule(LightningDataModule):
    ...
    def setup(self, stage: str):
        ...
        input_texts = [generate_prompt(prompt=self.prompt, question=question) for question in questions]
    ...
    def val_dataloader(self):
        def collate_fn(batch):
            ...
            input_ids = torch.stack([item["data"]["input_ids"].squeeze(0) for item in batch])
            attention_mask = torch.stack([item["data"]["attention_mask"].squeeze(0) for item in batch])
        ...
        return DataLoader(self.triviaQA_val, batch_size=self.batch_size, collate_fn=collate_fn)
